...
class SingleUTube(Borehole):
    ...
    @classmethod
    def from_dimensions(cls, R_d, H, D, r_b, x, y, basis, n_segments, tilt=0., orientation=0., segment_ratios=None, order=None):
        xi = jnp.array([-1., 1.])
        p = jnp.array(
            [
                [x, y, -D],
                [x + H * jnp.sin(tilt) * jnp.cos(orientation), y + H * jnp.sin(tilt) * jnp.sin(orientation), -D - H * jnp.cos(tilt)],
            ]
        )
