...
class LoadHistoryReconstruction(LoadAggregation):
    ...
    @staticmethod
    @jit
    def reconstruct_load_history(time, q):
    ...
    def set_current_load(self, q):
        self.q = self.q.at[self._k].set(q)
        return

    def temperature(self):
        self.q_reconstructed = self.reconstruct_load_history(self.time[:self._k+1], self.q[:self._k+1])
